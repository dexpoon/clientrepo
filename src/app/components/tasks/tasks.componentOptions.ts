...
//================================================
import { Category, Priority, InfoCategories, InfoPriorities } from '../common/containers';
...
export class TasksComponentOptions implements OnInit {
  ...
  showActives = false;   // to show Task, Aptm and Bill count
  showPassives = true;  // Do not show Infos and HowTos
  ...
  public selectedPriority: Priority = this.priorities[0];
  ...
  public isChecked = false; // search Description Only
  changed = (evt) => {
  ...
  ngOnInit() {
    ...
    this.initMaps();
    this.service.loadWithOption('PASSIV').
      subscribe(tasks => {
    ...
    //this.sortTable('prioritySorter'); so that last edited shows up first
    this.isChecked = false;
  ...
  HowtoCount() {
    let res = 0;
    for (let i = 0; i < this.tasks.length; i++) {
      if (this.tasks[i].category === 'HowTo')
        res++;
    }
    return res;
  }
  ...
  AssetCount() {
    let res = 0;
    for (let i = 0; i < this.tasks.length; i++) {
      if (this.tasks[i].category === 'Asset')
        res++;
    }
    return res;
  }
  ...
  format(objs) {
    for (var i = 0; i < objs.length; i++) {
      let tempDscr = objs[i].description;
      let tempNotes = objs[i].notes;

      objs[i].description = tempDscr.replace(/~/g, "-");
      objs[i].description = tempDscr.replace(/<>/g, "/");
  ...
  */
  ...
  initMaps() {
    if (!this.isMapInit) {
      ...
      this.CATEGORY_TO_COLOR_MAP.set('Info', 'LightSkyBlue');
      this.CATEGORY_TO_COLOR_MAP.set('HowTo', 'SpringGreen');
      this.CATEGORY_TO_COLOR_MAP.set('Bill', 'Violet');
